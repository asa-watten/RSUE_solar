...
dat <- fread("../3_output_data/ct_ma_2010_to_2019_sales.csv")
...
# matchmat <- fread("../1_input_data/CT_data_crosswalk.csv")
# matchmat <- matchmat[ProjectNumber!="" &
#                        !is.na(fips),
#                      c("ProjectNumber","P-ID-IRIS-FRMTD","fips","APN")]
# matchmat <- matchmat[!duplicated(matchmat)]
# fwrite(matchmat,"../1_input_data/CT_solar_ev_matching.csv")
...
# Add in missing zip-years
dat <- merge(dat,missingZipYr[,c("zip","year","missing")],
             by.x=c("zip","year.dat"),
             by.y=c("zip","year"),
             all.x=T)
dat$missing[is.na(dat$missing)] <- 1
...
# save -------------------------
fwrite(dat,"../3_output_data/merged_2010_2019_data.csv");beep(3)